function format_figure_acoustics(ax, formantStr,varargin)
...
switch formantStr
    case 'F1F2'
        ...
        if ConstrAxLim
        set (gca, 'XLim', [1000 2500], 'YLim', [0 1500])
        end
        xlabel ('F2 [Hz]','FontSize',16)
        ylabel ('F1 [Hz]','FontSize',16)
    case 'F2F3'
        set (gca, 'XDir', 'reverse')
        if ConstrAxLim
        set (gca, 'XLim', [1000 2500], 'YLim', [2000 3500])
        end
        xlabel ('F2 [Hz] ','FontSize',16)
        ylabel ('F3 [Hz]','FontSize',16)
        case 'F1F3'
        set (gca, 'YDir', 'reverse')
        if ConstrAxLim
        set (gca, 'XLim', [2000 3500], 'YLim', [0 1500])
        end
        ylabel ('F1 [Hz]','FontSize',16)
        xlabel ('F3 [Hz]','FontSize',16)
    otherwise
