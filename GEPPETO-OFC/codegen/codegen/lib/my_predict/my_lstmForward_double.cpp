...
// Include Files
#include "my_lstmForward_double.h"
#include <algorithm>
#include <cmath>
#include <cstring>
...
void lstmForwardUsingExplicitLoops(const double X[], const double W[],const double inputStateWeights[],
                                   const double R[],  const double recurrentStateWeights[], const double gate_bias[], const double stateBias[],
                                   double **CS, double **HS, int step)
{
  double G[4*LSTM_SIZE];
  double cellGateOp[LSTM_SIZE];
  double forgetGateOp[LSTM_SIZE];
  double ipGateOp[LSTM_SIZE];
  double outputGateOp[LSTM_SIZE];
  double f;
  double f1;
  double f2;
  double f3;
  ...
  int i;
  int p;
  std::memset(G, 0, 4*LSTM_SIZE * sizeof(double));
  ...
  */
   ...
   for (i = 0; i < LSTM_SIZE; i++) {
    ...
    for (p = 0; p < 10; p++) {
      ...
      f += W[b_i] * f4;
      f1 += W[b_i + LSTM_SIZE] * f4;
      f2 += inputStateWeights[i + LSTM_SIZE * p]*f4;
      f3 += W[b_i + 2*LSTM_SIZE] * f4;
    }
    for (p = 0; p < LSTM_SIZE; p++) {
      //b_i = i + (p << 6);
	    b_i = i + 3*LSTM_SIZE * p;
      ...
      f += R[b_i] * f4;
      f1 += R[b_i + LSTM_SIZE] * f4;
      f2 += recurrentStateWeights[i + LSTM_SIZE * p]*f4;
      f3 += R[b_i + 2*LSTM_SIZE] * f4;
    }
    G[i] = 1.0F / (std::exp(-(f + gate_bias[i])) + 1.0F);
    G[i + LSTM_SIZE] = 1.0F / (std::exp(-(f1 + gate_bias[i + LSTM_SIZE])) + 1.0F);
    G[i + 2*LSTM_SIZE] = std::tanh(f2 + stateBias[i]);
    G[i + 3*LSTM_SIZE] = 1.0F / (std::exp(-(f3 + gate_bias[i + 2*LSTM_SIZE])) + 1.0F);
