...
//#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
    p, f, f1, f2, f3, b_i, f4)
      ...
      /* SHOULD BE USELESS BY NOW -- AND PARAM actual ALSO [PB]
resetLSTM |= (step ==1);
if (resetLSTM && saved == 0) {
	memset(HS[step], 0, LSTM_SIZE * sizeof(double));
	memset(CS[step], 0, LSTM_SIZE * sizeof(double));
	resetLSTM = false;
}
if (resetLSTM && saved == 1) {
	if (actual) {
	veccopy(HS[step], saved_HS_Plant, 0, LSTM_SIZE-1);
	veccopy(CS[step], saved_CS_Plant, 0, LSTM_SIZE-1);}
	else {
	veccopy(HS[step], saved_HS_Kalman, 0, LSTM_SIZE-1);
	veccopy(CS[step], saved_CS_Kalman, 0, LSTM_SIZE-1);}
	resetLSTM = false;
}
*/
